(function () {
  ...
  angular
    ...
    }).controller('WorkflowsEditController', WorkflowsEditController)
      .run(function(Idle){
        Idle.watch();
      });

  WorkflowsEditController.$inject = ['$scope', '$state' ,'$window', 'Authentication', 'workflowResolve',
  ...
  function WorkflowsEditController($scope, $state , $window, Authentication, workflow,
    Modelfactory, flowchartConstants, $modal, $http, WKdraftService,WorkflowsValidationService, $interval, Constants, EnterpriseManager) {
    ...
    var nextNodeID = 0,
     // var nextNodeID = workflow.data.length + 1,
      ...
      currentDraftID = workflow.draftId,
      connectorsMappingList = [],
      edgeMappingList = [];
    var stateCount = workflow.data.length + 1; 
    ...
    vm.saveWorkflowModal = function (size, parentSelector, $document,workflow) {
      ...
      var parentElem = parentSelector ? angular.element($document[0].querySelector('.modal-demo ' + parentSelector)) : undefined,
        modalInstance = $modal.open({
          ...
          controller: 'WorkflowEditSettingModalController',
          controllerAs: 'vm',
          backdrop: 'static',
          size: size,
          scope: $scope,
  ...
  angular
    .module('workflows')
    .controller('WorkflowEditSettingModalController', WorkflowEditSettingModalController);

  WorkflowEditSettingModalController.$inject = ['$modalInstance', '$scope'];

  function WorkflowEditSettingModalController($modalInstance, $scope,workflow) {
    var vm = this;
